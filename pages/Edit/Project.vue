<template>
  <v-app>
    <v-container>
      ...
      <label>ゴミの量</label>
      <v-text-field v-model="name" class="white" placeholder="5kg"></v-text-field>
      <label>場所</label>
    ...
    <v-container>
      <v-row v-for="article in articles" :key="article.id" class="ma-1">
        <v-card width="100%">
          <v-row v-for="currentUser in currentUser" :key="currentUser.id">
            <v-col cols="3">
              <v-img :src="article.image" max-height="100" max-width="200" class="ml-1"></v-img>
            </v-col>
            <v-col cols="3">
            ...
            <v-col cols="1">
              <v-btn @click="openModalForEdit(article.id)" class="float-right">編集</v-btn>
              ...
              <v-overlay :value="editOverlay">
                ...
                <v-textarea v-model="comment" placeholder="コメントを入力"></v-textarea>
                <v-btn @click="editArticles(articleId)">編集</v-btn>
                <v-btn @click="closeModalForEdit">閉じる</v-btn>
              </v-overlay>
            </v-col>
            <v-col cols="1">
              <v-btn @click="openModalForDelete(article.id)" class="float-right">削除</v-btn>
