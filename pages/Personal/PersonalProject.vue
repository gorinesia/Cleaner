<template>
  <v-app>
    ...
    <v-alert
      ...
      :value="alertDelete"
      style="position: fixed; z-index: 1; right: 200px; bottom: 100px"
      >プロジェクトの様子を削除しました</v-alert
    >

    <!-- <ProjectArticle :key="article.id" :id="article.id" :article="article" /> -->

    <v-container
      v-for="personalProject in personalProject"
    ...
    >
        ...
        <v-row cols="12" md="2">
          ...
          <v-col cols="12" md="5">
            <v-list-item color="rgba(0, 0, 0, .4)">
              <v-list-item-content>
                <nuxt-link
                  class="display-name font-weight-bold title text-h6 text-decoration-none"
                  to="/personal/profile"
                  dark
                >
          ...
          <v-col cols="12" md="4">
            <span class="grey--text float-right mr-5"
              ><v-icon>mdi-scale</v-icon>{{ personalProject.amount }}・{{
              ...
              <v-list>
                      ...
                      <v-card>
                        ...
                        <v-card-text>
                          <v-container>
                            ...
                            ></v-text-field>
                            <label>日時</label>
                            <v-text-field v-model="time" class="white">{{
                              time
                            }}</v-text-field>
                            <label>場所</label>
                            <v-text-field
                ...
                <v-list-item>
                  ...
                  <v-list-item-title
                    ...
                    @click="openModalForDelete(personalProject.id)"
                    >投稿を削除</v-list-item-title
                  >
                </v-list-item>
              </v-list>
            </v-menu>
      ...
      <v-card align="center" class="mb-10 pa-5">
        ...
        <v-img height="300" width="500" :src="personalProject.image"></v-img>
      </v-card>
            ...
            <v-col>
              <v-avatar v-for="image in images" :key="image.id">
                <img
                  ...
                  width="50px"
                  height="50px"
                  @click="getProfile(personalProject.id)"
                />
              </v-avatar>
            </v-col>
          </v-row>
      ...
      <v-container class="h-full flex flex-col">
        <v-card class="h-full flex flex-col">
          ...
          <v-card class="border border-gray-900 rounded mb-4">
            ...
            ></v-textarea>
            <v-btn
              class="cyan text-sm white--text font-bold py-1 px-2 rouded"
              @click="addComment(currentUser[0].id)"
              >送信</v-btn
            >
          </v-card>
        </v-card>
        ...
        <v-row>
          <v-col cols="12" md="9">
            <span class="mb-3"
              ...
              >さんの行動に対してジュースを渡す(100円)感覚でサポートすることができます。</span
            >
            <v-dialog v-model="payDialog" persistent max-width="290">
              ...
              <v-card>
                ...
                <v-card-text
                  ...
                  >さんへ<br />「お疲れさま」の気持ちを込めて、ジュース代をプレゼントします。</v-card-text
                >
                <v-card-actions>
                  <v-spacer></v-spacer>
